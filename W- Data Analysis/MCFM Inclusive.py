...
import shutil
import uproot
datafile = "DYTurbo wm/results_wm-2d-nnlo-vj-msht20nnlo-member0-scetlibmatch.txt"
# Set the colour palette to "bright"
sns.set_palette("bright")
folders = ["Figure MCFM Inclusive", "Result MCFM Inclusive", "Figure DYTurbo wm", "Figure DYTurbo res"]
...
# make -j256 ; export OMP_STACKSIZE=512000 ; ./mcfm input_W-_Inclusive.ini
# loginctl enable-linger
# systemd-run --scope --user screen -S Session0

# General information
nproc = "6" # 31 (Z), 1 (W+), 6(W-)
...
def sel_files(parts_cp):
    ...
    for part in parts_cp:
        files_cp_pT.append("Result MCFM Inclusive/W-_data_6_" + part + "_none_0.001_pT.txt")
        files_cp_eta.append("Result MCFM Inclusive/W-_data_6_" + part + "_none_0.001_eta.txt")
...
# original_file_pT = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_pt34_fine.txt"
# original_file_eta = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_y34.txt"

# original_files = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_pt34_{i}.txt" for i in range(3, 36+1)]
...
# print("cd Wm13TeV ;", " ".join(copy_commands), "cd ..")

# # Copy the transverse momentum data and rapidity data to the destination folder with the new names
# shutil.copy("/eos/home-c/cyue/" + original_file_pT, "Result MCFM Inclusive/W-_data_{}_pT.txt".format(name))
# shutil.copy("/eos/home-c/cyue/" + original_file_eta, "Result MCFM Inclusive/W-_data_{}_eta.txt".format(name))
# i = 3
# for file in original_files:
#     shutil.copy("/eos/home-c/cyue/" + file, f"Result MCFM Inclusive/W-_data_{name}_pT_{i}.txt")
#     i += 1
# print(i)

# Define the bins
bins_pT = np.arange(0, 101)
bins_eta = np.append(np.insert(np.arange(-4, 4.25, 0.25), 0, -5), 5)
...
def extract(file):
    ...
    with open(file, "r") as f:
        if file.endswith("pT.txt") or any(file.endswith(f"pT_{i}.txt") for i in range(3, 36+1)) or file.endswith("eta.txt"):
            # Skip the first 5 rows
            for i in range(5):
                f.readline()
            # Extract the data
            data = [line.split() for line in f.readlines()]
            if file.endswith("pT.txt") or any(file.endswith(f"pT_{i}.txt") for i in range(3, 36+1)):
    ...
    if not file.startswith(".sys."):
        if file.endswith("pT.txt") or file.endswith("eta.txt"):
            ...
            if "NNPDF31" in file or "MSHT20" in file:
                ...
                part = file.split("_")[4]
                name = file[len(f"Result MCFM Inclusive/W-_data_{lhapdf}_"):-len(".txt")]
            else:
                part = file.split("_")[3]
                name = file[len(f"Result MCFM Inclusive/W-_data_"):-len(".txt")]
            ...
            if file.endswith("pT.txt"):
                plt.xlabel(r"$p_T^W$ (GeV)")
                plt.ylabel(r"$\sigma$ (pb)")
                plt.title(fr"$\sigma$ vs $p_T^W$")
                plt.legend()
                ...
                plt.xlabel(r"$Y^W$")
                plt.ylabel(r"$\sigma$ (pb)")
                plt.title(fr"$\sigma$ vs $Y^W$")
                plt.legend()
                plt.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=plt.gca().transAxes, ha="right", va="top")
                plt.savefig(f"Figure MCFM Inclusive/sigma vs abs(Y) for {name}.png")
                plt.show()
            ...
            plt.legend()
            plt.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=plt.gca().transAxes, ha="right", va="top")
            plt.savefig(f"Figure DYTurbo wm/sigma vs pT for {label}.png")
            plt.show()
            fig = plt.figure(figsize=(8, 6))
            ...
            plt.legend()
            plt.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=plt.gca().transAxes, ha="right", va="top")
            plt.savefig(f"Figure DYTurbo wm/sigma vs abs(Y) for {label}.png")
            plt.show()

for file in os.listdir("DYTurbo wm"):
    plot(file)
...
def compare_pT(parts_cp):
    ...
    for i, file in enumerate(files_cp_pT):
        part = file.split("_")[3]
        name = file[len("Result MCFM Inclusive/W-_data_"):-len(".txt")]
        label = part_dict[part]
        edges, values, errors = extract(file)
        edges_centres = (edges[:-1] + edges[1:]) / 2
        yerr = errors
    ...
    ax1.set_xlabel(r"$p_T^W$ (GeV)")
    ax1.set_ylabel(r"$\sigma$ (pb)")
    ax1.set_title(fr"$\sigma$ vs $p_T^W$")
    ax1.legend()
    ...
    ax2.set_xlabel(r"$p_T^W$ (GeV)")
    ax2.set_ylabel("Pred./DYTurbo")
#     ax2.legend()
    ax2.set_xlim(edges_pT[0], edges_pT[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wm/sigma vs pT for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
def compare_eta(parts_cp):
    ...
    for i, file in enumerate(files_cp_eta):
        ...
        ax2.step(edges, ratios, where="post", linestyle="-", label=label, color=colors[i+1])
        ax2.fill_between(edges, ratios - ratios_errors, ratios + ratios_errors, alpha=0.3, step="post", linewidth=0, color=colors[i+1])
    ax1.set_xlabel(r"$Y^W$")
    ax1.set_ylabel(r"$\sigma$ (pb)")
    ax1.set_title(fr"$\sigma$ vs $Y^W$")
    ...
    ax2.axhline(1.0, linestyle="--")
    ax2.set_xlabel(r"$Y^W$")
    ax2.set_ylabel("Pred./DYTurbo")
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wm/sigma vs Y for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
def compare_norm_eta(parts_cp):
    ...
    ax1.set_title(r"Normalized $\sigma$ vs $Y^W$")
    ax1.legend(loc="lower center")
    ax1.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=ax1.transAxes, ha="right", va="top")
    ax1.text(0, 1.04, "CMS", fontname="Arial", fontsize=16, transform=ax1.transAxes, fontweight="bold", va="top", ha="left")
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wm/Normalized sigma vs Y for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
def hist_2d(part):
    ...
    for i in range(3, 36+1):
        files.append(f"W-_data_{name}_pT_{i}.txt")
...
def extract_bin(file):
    # Open the text file
    with open(file, "r") as f:
        if any(file.endswith(f"pT_{i}.txt") for i in range(3, 36+1)):
            # Skip the first 5 rows
            for i in range(5):
                f.readline()
            # Extract the data
...
#     fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 8), gridspec_kw={"height_ratios": [5, 1]})
#     edges_Wmunu, values_Wmunu, errors_Wmunu = extract_bin(datafile)
#     if "an3lo" in datafile:
#         member = datafile.split("-")[5].replace("member", "")
#         label_DYTurbo = f"DYTurbo an3lo {member}"
#     else:
#         member = datafile.split("-")[4].replace("member", "")
#         label_DYTurbo = f"DYTurbo {member}"
#     ax1.plot(np.arange(len(values_Wmunu)), values_Wmunu/bin_widths, linestyle="-", label=label_DYTurbo)
#     for part in parts_2d:
#         nproc, ewcorr, pgoal = "6", "none", "0.001"
#         label = part_dict[part]
#         name = "_".join([nproc, part, ewcorr, pgoal])
#         files = []
#         values_array = []
...
#     ax2.legend()
#     plt.tight_layout()
#     plt.savefig(f"Figure DYTurbo wm/sigma vs bin for {label_DYTurbo} {len(parts_cp)} sets.png")
#     plt.show()
#
...
res_path = "TheoryCorrections/scetlib_dyturboCorrW.pkl.lz4"
res_file = os.path.join(home, res_path)
res = pickle.load(lz4.frame.open(res_file))["W"]["scetlib_dyturbo_hist"][{"vars": "pdf0"}]
res = res.values()[:,:,:,0]
# sudo umount -f /Volumes/wmass ; sudo mkdir -p /Volumes/wmass ; sudo sshfs -o reconnect -o follow_symlinks -o allow_other cyue@lxplus8s10.cern.ch:/home/c/cyue /Volumes/wmass/
# make -j256 ; export OMP_STACKSIZE=512000 ; ./mcfm input_W-_Inclusive.ini
# loginctl enable-linger
...
def sel_files_res(parts_cp):
    ...
    for part in parts_cp:
        for lhapdf in lhapdfs:
            files_cp_res_pT.append(f"Result MCFM Inclusive/W-_data_{lhapdf}_6_" + part + "_none_0.001_pT.txt")
            files_cp_res_eta.append(f"Result MCFM Inclusive/W-_data_{lhapdf}_6_" + part + "_none_0.001_eta.txt")
    return files_cp_res_pT, files_cp_res_eta

# # Get the command to copy the file to /eos/home-c/cyue
# suffix = "_1.0E-4" if ewcorr == "none" and part == "nnlo" or part == "nnlo" else ""
# original_file_pT_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_pt34_fine.txt"
# original_file_eta_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_y34.txt"

# original_files_res = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wm13TeV_pt34_{i}.txt" for i in range(3, 18+1)]
...
# Get the command to copy the file to /eos/home-c/cyue
suffix = "_1.0E-4" if ewcorr == "none" and part == "nnlo" or part == "nnlo" else ""
original_file_pT_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wm13TeV_pt34_fine.txt"
original_file_eta_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wm13TeV_y34.txt"

original_files_res = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wm13TeV_pt34_{i}.txt" for i in range(3, 18+1)]
...
# # Copy the transverse momentum data and rapidity data to the destination folder with the new names
# shutil.copy("/eos/home-c/cyue/" + original_file_pT_res, "Result MCFM Inclusive/W-_data_NNPDF31_{}_pT.txt".format(name))
# shutil.copy("/eos/home-c/cyue/" + original_file_eta_res, "Result MCFM Inclusive/W-_data_NNPDF31_{}_eta.txt".format(name))
# i = 3
# for file in original_files_res:
#     shutil.copy("/eos/home-c/cyue/" + file, f"Result MCFM Inclusive/W-_data_NNPDF31_{name}_pT_{i}.txt")
#     i += 1
# print(i)

# # Copy the transverse momentum data and rapidity data to the destination folder with the new names
# shutil.copy("/eos/home-c/cyue/" + original_file_pT_res, "Result MCFM Inclusive/W-_data_MSHT20_{}_pT.txt".format(name))
# shutil.copy("/eos/home-c/cyue/" + original_file_eta_res, "Result MCFM Inclusive/W-_data_MSHT20_{}_eta.txt".format(name))
# i = 3
# for file in original_files_res:
#     shutil.copy("/eos/home-c/cyue/" + file, f"Result MCFM Inclusive/W-_data_MSHT20_{name}_pT_{i}.txt")
#     i += 1
# print(i)
...
#     ax2.legend()
    ...
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/sigma vs pT{'' if pT_low is None else pT_low} for DYTurbo resummed {len(parts_cp)} sets W-.png")
    plt.show()

compare_res_pT(parts_cp, None)
compare_res_pT(parts_cp, 20)
...
def comb_resNNLO_pT(lhapdf, pT_low):
    ...
    edges_pT = bins_res_pT
    edges_pT_centres = (edges_pT[:-1] + edges_pT[1:]) / 2
    edges_pT, values_pT, errors_pT = extract(f"Result MCFM Inclusive/W-_data_{lhapdf}_6_resNNLO_none_0.01_pT.txt")
...
#     ax2.legend()
    ax2.set_xlim(edges_pT[0], edges_pT[-1] if pT_low is None else pT_low)
    plt.tight_layout()
    plt.savefig(f"Figure MCFM Inclusive/sigma vs pT{'' if pT_low is None else pT_low} for resNNLO {lhapdf} W-.png")
    plt.show()
...
#     ax2.set_ylim(0.7, 1.15)
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/Normalized sigma vs pT{'' if pT_low is None else pT_low} for DYTurbo resummed {len(parts_cp)} sets W-.png")
    plt.show()
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/sigma vs abs(Y) for DYTurbo resummed {len(parts_cp)} sets W-.png")
    plt.show()
...
def comb_resNNLO_eta(lhapdf):
    ...
    edges_eta = bins_res_eta
    edges_eta_centres = (edges_eta[:-1] + edges_eta[1:]) / 2
    edges_eta, values_eta, errors_eta = extract(f"Result MCFM Inclusive/W-_data_{lhapdf}_6_resNNLO_none_0.01_eta.txt")
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure MCFM Inclusive/sigma vs abs(Y) for resNNLO {lhapdf} W-.png")
    plt.show()
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/Normalized sigma vs abs(Y) for DYTurbo resummed {len(parts_cp)} sets W-.png")
    plt.show()
...
def plot_vs_res_DYTurbo(parts_cp):
    ...
    values_Wmunu = res.T.flatten()
    label_DYTurbo_res = r"DYTurbo resummed, $\mathrm{N}^{3}$LL+NNLO"
    ax1.plot(np.arange(len(values_Wmunu)), values_Wmunu/bin_res_widths, linestyle="-", label=label_DYTurbo_res)
        ...
        try:
            for part in parts_cp:
                ...
                for i in range(3, 18+1):
                    files.append(f"W-_data_{lhapdf}_{name}_pT_{i}.txt")
                files = check_pgoal(files)
                for file in files:
                    if not file.startswith("Result MCFM Inclusive/"):
                        file = "Result MCFM Inclusive/" + file
    ...
    ax1.text(0, 1.04, "CMS", fontname="Arial", fontsize=16, transform=ax1.transAxes, fontweight="bold", va="top", ha="left")
    ax1.text(0.085, 1.04, "Preliminary", fontname="Arial", fontsize=14, transform=ax1.transAxes, style="italic", va="top", ha="left")
    ax1.set_xlim(0, len(values_Wmunu))
    ax2.set_xlabel("bin")
    ax2.set_ylabel("Pred./DYTurbo resummed")
#     ax2.legend()
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/sigma vs bin for DYTurbo resummed {len(parts_cp)} sets W-.png")
    plt.show()
...
def comb_resNNLO_bin(lhapdf):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 8), gridspec_kw={"height_ratios": [5, 1]}, sharex=True)
    values_Wmunu = np.array([])
    errors_Wmunu = np.array([])
    values_sum = np.array([])
    errors_sum = np.array([])
    for part in ["resNNLO", "resonlyNNLO", "resexpNNLO", "resaboveNNLO"]:
        ...
        if part == "resNNLO":
            ...
            if len(values_sum) == 0:
                ...
                values_sum += values
                errors_sum += errors
    ax1.plot(np.arange(len(values_Wmunu)), values_Wmunu/bin_res_widths, linestyle=":", label=f"resNNLO ({lhapdf})")
    ...
    ax1.set_xlim(0, len(values_Wmunu))
    next(ax2._get_lines.prop_cycler)
    ratios = values_sum/values_Wmunu
...
#     ax2.legend()
    plt.tight_layout()
    plt.savefig(f"Figure MCFM Inclusive/sigma vs bin for resNNLO {lhapdf} W-.png")
    plt.show()
...
def chi_sq(part, lhapdf, pT_low):
    ...
    values_Wmunu = res.T.flatten()[:cut]
    nproc, ewcorr, pgoal = "6", "none", "0.001"
    label = part_dict[part]
    name = "_".join([nproc, part, ewcorr, pgoal])
    files = []
    ...
    chi_sq = ((values_Wmunu-values)**2/errors**2)
    chi_sq = np.sum(chi_sq[chi_sq <= 1e5])/len(chi_sq)
    return chi_sq
