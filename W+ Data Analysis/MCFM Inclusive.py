...
import shutil
import uproot
datafile = "DYTurbo wp/results_wp-2d-nnlo-vj-msht20nnlo-member0-scetlibmatch.txt"
# Set the colour palette to "bright"
sns.set_palette("bright")
folders = ["Figure MCFM Inclusive", "Result MCFM Inclusive", "Figure DYTurbo wp", "Figure DYTurbo res"]
...
def sel_files(parts_cp):
    ...
    for part in parts_cp:
        files_cp_pT.append("Result MCFM Inclusive/W+_data_1_" + part + "_none_0.001_pT.txt")
        files_cp_eta.append("Result MCFM Inclusive/W+_data_1_" + part + "_none_0.001_eta.txt")
...
# original_file_pT = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_pt34_fine.txt"
# original_file_eta = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_y34.txt"

# original_files = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_pt34_{i}.txt" for i in range(3, 36+1)]
...
# print("cd Wp13TeV ;", " ".join(copy_commands), "cd ..")
...
def plot(file):
    if not file.startswith(".sys."):
        if file.endswith("pT.txt") or file.endswith("eta.txt"):
            ...
            plt.legend()
            plt.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=plt.gca().transAxes, ha="right", va="top")
            plt.savefig(f"Figure DYTurbo wp/sigma vs pT for {label}.png")
            plt.show()
            fig = plt.figure(figsize=(8, 6))
            ...
            plt.legend()
            plt.text(1, 1.05, r"16.8 fb$^{-1}$", fontname="Arial", fontsize=16, transform=plt.gca().transAxes, ha="right", va="top")
            plt.savefig(f"Figure DYTurbo wp/sigma vs Y for {label}.png")
            plt.show()
            
for file in os.listdir("DYTurbo wp"):
    plot(file)
...
#     ax2.legend()
    ...
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wp/sigma vs pT for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
#     ax2.legend()
    ...
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wp/sigma vs Y for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo wp/Normalized sigma vs Y for {label_DYTurbo} {len(parts_cp)} sets.png")
    plt.show()
...
#     for part in parts_2d:
#         nproc, ewcorr, pgoal = "1", "none", "0.001"
#         label = part_dict[part]
#         name = "_".join([nproc, part, ewcorr, pgoal])
#         files = []
#         values_array = []
...
#     ax2.legend()
#     plt.tight_layout()
#     plt.savefig(f"Figure DYTurbo wp/sigma vs bin for {label_DYTurbo} {len(parts_cp)} sets.png")
#     plt.show() 
...
def sel_files_res(parts_cp):
    ...
    for part in parts_cp:
        for lhapdf in lhapdfs:
            files_cp_res_pT.append(f"Result MCFM Inclusive/W+_data_{lhapdf}_1_" + part + "_none_0.001_pT.txt")
            files_cp_res_eta.append(f"Result MCFM Inclusive/W+_data_{lhapdf}_1_" + part + "_none_0.001_eta.txt")
...
# # Get the command to copy the file to /eos/home-c/cyue
# suffix = "_1.0E-4" if ewcorr == "none" and part == "nnlo" or part == "nnlo" else ""
# original_file_pT_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_pt34_fine.txt"
# original_file_eta_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_y34.txt"

# original_files_res = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}NNPDF31_nnlo_as_0118_1.00_1.00{suffix}_Wp13TeV_pt34_{i}.txt" for i in range(3, 18+1)]
...
# Get the command to copy the file to /eos/home-c/cyue
suffix = "_1.0E-4" if ewcorr == "none" and part == "nnlo" or part == "nnlo" else ""
original_file_pT_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wp13TeV_pt34_fine.txt"
original_file_eta_res = f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wp13TeV_y34.txt"

original_files_res = [f"W_only_{part}_{ewcorr if ewcorr != 'none' else ''}MSHT20nnlo_as118_1.00_1.00{suffix}_Wp13TeV_pt34_{i}.txt" for i in range(3, 18+1)]
...
def comb_resNNLO_pT(lhapdf, pT_low):
    ...
    edges_pT = bins_res_pT
    edges_pT_centres = (edges_pT[:-1] + edges_pT[1:]) / 2
    edges_pT, values_pT, errors_pT = extract(f"Result MCFM Inclusive/W+_data_{lhapdf}_1_resNNLO_none_0.01_pT.txt")
...
def comb_resNNLO_eta(lhapdf):
    ...
    edges_eta = bins_res_eta
    edges_eta_centres = (edges_eta[:-1] + edges_eta[1:]) / 2
    edges_eta, values_eta, errors_eta = extract(f"Result MCFM Inclusive/W+_data_{lhapdf}_1_resNNLO_none_0.01_eta.txt")
...
#     ax2.legend()
    ax2.set_xlim(edges_eta[0], edges_eta[-1])
    plt.tight_layout()
    plt.savefig(f"Figure DYTurbo res/Normalized sigma vs abs(Y) for {label_DYTurbo_res} {len(parts_cp)} sets W+.png")
    plt.show()
...
def chi_sq(part, lhapdf, pT_low):
    ...
    chi_sq = ((values_Wmunu-values)**2/errors**2).sum()/len(values)
    return chi_sq
